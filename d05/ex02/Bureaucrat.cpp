...
void Bureaucrat::signForm(Form &src) {
	if (src.getGrSign() <= this->getGrade())
		throw GradeTooLowException();
//		std::cout << "<" << this->getName() << "> cannot sign <"
//				  << src.getName() << "> because his grade is too low.\n";
	else {
...
void Bureaucrat::execute(Form &src) const {
	if (this->getGrade() > src.getGrExec())
		throw Bureaucrat::GradeTooLowException();
	std::cout << "<" << this->getName() << "> executed <" << src.getName() << "> form.\n";
	src.execute(*this);
}